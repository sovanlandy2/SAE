...
def classify2(text):
    ...
    pos_polarity = sum((pos[word] + 1) / (2 * totals[0]) for word in words)
    neg_polarity = sum((neg[word] + 1) / (2 * totals[1]) for word in words)
    while (pos_polarity < 0.1 ): 
        pos_polarity*=10
    while (neg_polarity < 0.1 ): 
        neg_polarity*=10
    return (pos_prob > neg_prob, abs(pos_prob - neg_prob), pos_polarity, neg_polarity)
